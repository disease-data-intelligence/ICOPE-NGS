...
First, let's define bash variables for keeping our workspace clean.  
        ...
        ## Tools, input files and resources 
...
#### 3.1 - MuTect2
_All chromosomes took ~327 minutes, (~5.4 hours with one processor)_
...
from the 50.000.000th to the 52.000.000th base pair, which is set with the `-L` option. 
...
* Where do you find the most variants and does that make biological sense? What is the difference in the two numbers and does it match above?  
...
You should see this line (without the header):

        #CHROM  POS     ID      REF     ALT     QUAL    FILTER  INFO    FORMAT  TCRBOA2-N-WEX   TCRBOA2-T-WEX
        chr1    50973993        .       C       T       .       PASS    CONTQ=93;DP=283;ECNT=1;GERMQ=93;MBQ=29,23;MFRL=192,183;MMQ=60,60;MPOS=16;NALOD=2.07;NLOD=34.30;POPAF=4.61;SEQQ=16;STRANDQ=50;TLOD=6.85  GT:AD:AF:DP:F1R2:F2R1:SB      0/0:133,0:8.437e-03:133:57,0:76,0:57,76,0,0     0/1:130,6:0.046:136:64,2:66,3:64,66,3,3'
...
After genotype (GT) we have allelic depth (AD) which is "130,6" (i.e. 130 reads and 6 reads for
the reference and mutant allele respectively) in the tumor and "133,0" in the normal sample. Then comes allelic frequency, which is
...
Try to search [dbSNP](https://www.ncbi.nlm.nih.gov/snp) for rs746646631. 
